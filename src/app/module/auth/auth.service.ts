...
import { IChangePasswordPayload, ICreateUserPayload, IForgotPasswordPayload, ILoginMeta, ILoginUserPayload, IResetPasswordPayload, IRevokeSessionPayload, IUpdateProfilePayload } from "./auth.interface";
...
const verifyEmail = async (token: string) => {
    ...
    if (verification.expiresAt < new Date()) {
        ...
        throw new AppError(status.BAD_REQUEST, "Verification token has expired");
    }
...
const sendVerificationEmail = async (payload: { email: string }) => {
    ...
    const verificationUrl = `${env.FRONTEND_URL}/auth/sign-up?email=${email}&token=${rawToken}`;
...
const signUpUser = async (payload: ICreateUserPayload) => {
    ...
    if (verification.identifier !== expectedIdentifier) {
        throw new AppError(status.BAD_REQUEST, "Verification token does not match the provided email");
    }
    ...
    if (isUserExist) {
        throw new AppError(status.BAD_REQUEST, "Username already exists with this username");
    }
    ...
    await prisma.session.create({
        data: {
            ...
            expiresAt: tokenUtils.getExpiresAtFromDuration(env.REFRESH_TOKEN_EXPIRES_IN),
        }
    });
...
const loginUser = async (payload: ILoginUserPayload, meta?: ILoginMeta) => {
    ...
    if (user.isDeleted || user.status === UserStatus.INACTIVE || user.status === UserStatus.DELETED) {
...
const getNewToken = async (refreshToken: string) => {
    ...
    const { userId } = verifiedRefreshToken.data as { userId: string };
    ...
    if (session.expiresAt < new Date()) {
        throw new AppError(status.UNAUTHORIZED, "Session has expired. Please log in again.");
    }

    const user = await prisma.user.findUnique({ where: { id: userId } });
    if (!user || user.isDeleted || user.status === UserStatus.INACTIVE || user.status === UserStatus.DELETED) {
        throw new AppError(status.UNAUTHORIZED, "User account is inactive or deleted");
    }
...
const forgotPassword = async (payload: IForgotPasswordPayload) => {
    ...
    if (!user) {
        throw new AppError(status.NOT_FOUND, "User not found with this username");
    }
...
const resetPassword = async (payload: IResetPasswordPayload) => {
    ...
    if (!verification.identifier.startsWith("password-reset:")) {
        throw new AppError(status.BAD_REQUEST, "Invalid reset token");
    }
...
const revokeSession = async (userId: string, payload: IRevokeSessionPayload) => {
    ...
    if (session.revokedAt) {
        throw new AppError(status.BAD_REQUEST, "Session is already revoked");
    }
